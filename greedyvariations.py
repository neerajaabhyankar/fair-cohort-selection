...
def greedy_prioritized_quota(oracle, memberships, unfilled_quotas, k, verbose=False):
    """
    :param memberships: n rows (members), p columns (0 or 1 membership bools)
    :param unfilled_quotas: indices in [n], numpy array of length p
    :param k: total capacity
    ...
    while np.sum(unfilled_quotas) > 0:
        ...
        most_unfilled_group = np.argmax(unfilled_quotas)
        V_quota_candidates = [ii for ii in V if memberships[ii][most_unfilled_group] > 0]
        if len(V_quota_candidates) <= 0:
            # we ran out of quota candidates for this group
            unfilled_quotas[most_unfilled_group] = 0
            continue
